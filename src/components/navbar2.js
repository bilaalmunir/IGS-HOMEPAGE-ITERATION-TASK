...
const Navbar2 = () => {
  ...
  const [drawerMenusOpen, setDrawerMenusOpen] = React.useState(Array(menuItems.length).fill(false));
  ...
  return (
    <AppBar position="static" color="transparent" sx={{ minHeight: 56, bgcolor: 'transparent' }} elevation={0}>
      ...
      <Box sx={{ height: { xs: 24, sm: 32, md: 40, lg: 48 }, width: 'auto', display: 'flex', alignItems: 'center' }}>
  <img src="/logo.png" alt="Company Logo" style={{ height: '100%', width: 'auto', display: 'block' }} />
</Box>
        {/* Desktop Menu */}
        {!isMobile && (
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 3 }}>
            {menuItems.map((item, idx) => (
              <Box key={item.label} sx={{ display: 'flex', alignItems: 'center' }}>
                <Typography variant="body1" color="white" sx={{ fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 1, fontSize: 14 }} onClick={(e) => handleMenuOpen(idx, e)}>
                  {item.label}
        ...
        <Drawer anchor="right" open={drawerOpen} onClose={handleDrawerToggle}>
          <Box sx={{ width: 250, bgcolor: '#111827', height: '100%' }} role="presentation" onClick={handleDrawerToggle}>
            <List>
              {menuItems.map((item, idx) => (
                <React.Fragment key={item.label}>
                  <ListItem button onClick={(e) => { e.stopPropagation(); handleDrawerMenuClick(idx); }} sx={{ color: 'white' }}>
                    <ListItemText primary={item.label} />
                    <ArrowDropDownIcon sx={{ color: 'white', transform: drawerMenusOpen[idx] ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }} />
                  </ListItem>
                  <Collapse in={drawerMenusOpen[idx]} timeout="auto" unmountOnExit>
                    <List component="div" disablePadding>
                      {item.menu.map((option) => (
                        <ListItem button key={option} sx={{ pl: 4, color: 'white' }}>
